...
- Each Sales row represents one item listing with an editable asking price, `selling_started_at`, and nullable `date_sold`; lot quantity is implicit and not part of the Sales workflow. Recording a price automatically creates a linked active listing, Duplicate copies the item and price, and marking sold moves it into history. Active and sold tables sort independently by any displayed data field. Row deletion requires browser confirmation and does not delete linked price history.
