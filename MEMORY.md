...
- The shared page header exposes Item Search as an active top-level tab and is structured for later page tabs. A blank item query renders the full alphabetical catalog; typing filters it by normalized name.
- Catalog rows include category, latest valid unit price, observed lot, and observation time for the active market. Prices are selected in one bulk window query, and every row cell opens item detail for append-only price entry rather than direct mutation.
