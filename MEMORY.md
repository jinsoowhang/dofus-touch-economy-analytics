...
- The Recipe Calculator accepts up to 100 latest-recipe items with craft quantities from 1 through 1,000. It aggregates shared resolved ingredients by canonical item identity, combines their required quantities, lists consuming crafts, and derives current unit prices, extended costs, price coverage, and complete or known-cost totals. Its compact choice list uses the same scalar recipe projection, and selected quantities persist in browser-local storage while remaining non-authoritative request state. Resolved shopping-list unit prices are inline-editable on Enter or blur; each save appends a quantity-one price observation, resubmits the current calculator selection, and recalculates every affected total without creating a Sales listing. Unresolved ingredients and missing prices remain explicit.
