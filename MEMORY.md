...
- The Recipe Calculator accepts up to 100 latest-recipe items with craft quantities from 1 through 1,000. It aggregates shared resolved ingredients by canonical item identity, combines their required quantities, lists consuming crafts, and derives current unit prices, extended costs, price coverage, and complete or known-cost totals. Cart membership, craft quantity, and calculation selection persist separately in browser-local storage while remaining non-authoritative request state. Row checkboxes plus Select all and Select none control the next calculation; unchecked items remain in the cart until explicitly removed. Resolved shopping-list unit prices are inline-editable on Enter or blur; each save appends a quantity-one price observation, resubmits the current calculator selection, and recalculates every affected total without creating a Sales listing. Unresolved ingredients and missing prices remain explicit.
