...
- The shared page header exposes Item Search, Recipes, and Sales as top-level tabs. A blank item query renders the full catalog; typing filters it by normalized name, and an optional exact Category filter combines with the name query. Item Search headers sort by name, category, current price, or observation date in either direction, with inactive headers starting descending, active filters preserved, and missing values kept last.
